...
- `start-reviewer.sh`: idempotently start or reuse a named reviewer agent. Handles reuse probes, platform differences (including Windows, where `herdr agent start --kind pi` cannot launch `.cmd` shims such as `pi.cmd`; the pane fallback invokes the explicit `.cmd` shim so Herdr recognizes Pi, then waits for detection and renames), and fallbacks.
